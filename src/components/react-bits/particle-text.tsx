 ...
 * src/content/TextAnimations/ParticleText) — pulled from source directly
 * because reactbits.dev's `/r/{name}.json` registry endpoint returns its SPA
 * shell instead of JSON to non-browser clients (curl, the shadcn CLI), and
 * the shadcn MCP server this repo now has configured (`.mcp.json`) needs a
 * Claude Code restart to connect. Logic is unchanged from upstream; only the
 * defaults (brand colors) and styling (Tailwind classes instead of a
 * co-located CSS file — the original's CSS has no radius/color baked in, so
 * it converts cleanly) changed. See TASK-react-bits-mcp-registry.md.
 */
...
async function waitForFonts(font: string) {
  if (!("fonts" in document)) return;
  try {
    await document.fonts.load(font);
  } catch {
    // Ignore — the canvas sampling below falls back to whatever font resolves.
  }
