/**
 * Headless ASCII-object engine: three.js scene, glyph atlas, and render loop,
 * with no React and no hard dependency on the DOM.
 *
 * Split out of `AsciiObject.tsx` so a worker can import it. That file keeps the
 * React component and re-exports everything here, so its public API is
 * unchanged. The split is at the React boundary rather than duplicating the
 * scene logic into a second worker-only implementation: the task doc originally
 * proposed two paths that "must stay behaviourally identical", which is a
 * standing bug risk in the most complex file in the repo. There is one
 * implementation, and it runs in either place.
 *
 * What that costs is a handful of injection points instead of direct global
 * reads — the canvas may be an `OffscreenCanvas`, size and DPR may be supplied
 * rather than measured, and `prefers-reduced-motion` may be pushed in. Each is
 * marked below. Behaviour on the main thread is unchanged: when handed a real
 * `HTMLCanvasElement` this file does exactly what it did before, observers and
 * `matchMedia` included.
 *
...
/**
 * Environment probes that survive the bundler.
 *
 * `typeof window === "undefined"` is exactly how Next strips server-only code,
 * so its client build folds that test to a constant — the guard disappears and
 * what is left is a bare `window.matchMedia(...)`, which throws
 * `ReferenceError: window is not defined` the moment this module is imported
 * into a worker. The failure is quiet: the host catches it and falls back to
 * the main thread, so the page looks perfect and the worker never runs. It cost
 * a full round of profiling to notice.
 *
 * Reading the properties off `globalThis` is not a style preference. It is the
 * form the compiler cannot prove anything about, so the check survives to
 * runtime. Use these two instead of bare `window` / `document` anywhere in this
 * file.
 */
...
export interface AsciiObjectOptions {
  ...
  yOffset?: number;
  /** Static rotation around the X axis, in degrees. The built-in camera sits below and in front of the object (CAMERA_DIR), so objects modeled facing forward at eye level often need a tilt here to present their front face instead of their underside. */
  rotateX?: number;
  /** Static rotation around the Y axis, in degrees. */
  rotateY?: number;
  /** Static rotation around the Z axis, in degrees. */
  rotateZ?: number;
...
/** CSS size and device pixel ratio of the display canvas. Supplied by the
 * caller when the canvas cannot measure itself — an `OffscreenCanvas` has no
 * layout box and a worker has no `window.devicePixelRatio`. */
...
export interface AsciiObjectElements {
  /** Canvas the scene renders to. Pass an `OffscreenCanvas` (transferred via
   * `transferControlToOffscreen`) to run the whole scene off the main thread;
   * `metrics` and `reducedMotion` then become required, since neither can be
   * derived without a DOM. */
  canvas: HTMLCanvasElement | OffscreenCanvas;
  /** Initial size/DPR. Ignored for a DOM canvas, which measures itself. */
  metrics?: CanvasMetrics;
  /** Resolved `prefers-reduced-motion`. Ignored for a DOM canvas, which
   * queries `matchMedia` itself and keeps listening for changes. */
  reducedMotion?: boolean;
}

export interface AsciiObjectTransform {
  scale?: number;
  xOffset?: number;
  yOffset?: number;
  rotateX?: number;
...
export interface AsciiObjectInstance {
  /** Update options live. Changing src loads the new asset. Not meant to be called every animation frame — it unconditionally resyncs the renderer and re-checks the asset, which reallocates GPU render targets each call. For continuous per-frame scroll/animation-driven updates, use setTransform instead. */
  setOptions: (options: AsciiObjectOptions) => void;
  /** Cheap, per-frame-safe update of scale/offset/rotation only — skips the resize and asset-reload work setOptions does. */
  setTransform: (transform: AsciiObjectTransform) => void;
  /** Re-read canvas size. Call when the element is resized. Off the main
   * thread there is nothing to re-read, so pass the new metrics. */
  resize: (metrics?: CanvasMetrics) => void;
  /** Start/stop the render loop. On the main thread an `IntersectionObserver`
   * does this automatically; a worker has no observer, so its host forwards
   * visibility across. */
  setVisible: (visible: boolean) => void;
  /** Update `prefers-reduced-motion` when the engine cannot observe it itself
   * (worker path). No-op on the main thread, which owns a live media query. */
...
/** A canvas used as a pixel buffer (glyph atlas, decoded image) rather than as
 * a display surface. In a worker there is no `document`, so it is an
 * `OffscreenCanvas`; both satisfy `CanvasTexture` and `drawImage`. */
...
function makeCanvas(width: number, height: number): RasterCanvas {
  ...
  if (!ENV.document) return new OffscreenCanvas(w, h);
...
async function decodeWithBitmap(blob: Blob): Promise<RasterCanvas | null> {
...
export function createAsciiObject(
  ...
  // The single branch that decides which environment we are in. A real
  // element means the main thread: measure it, observe it, listen to
  // matchMedia — exactly as before. Otherwise the host supplies those facts.
  const domCanvas =
    ENV.window && canvas instanceof ENV.window.HTMLCanvasElement ? canvas : null;
  let metrics: CanvasMetrics = elements.metrics ?? {
    width: 1,
    height: 1,
    pixelRatio: 1,
  };
  ...
  // OrbitControls binds DOM events and cannot exist in a worker. Callers that
  // want orbit/zoom/autoRotate must stay on the main thread; `createAsciiObject`
  // is unchanged there, controls included. See the worker host, which refuses
  // to take the offscreen path when any of those options are on.
  const controls = domCanvas ? new OrbitControls(camera, domCanvas) : null;
  if (controls) {
  ...
  let atlasTexture: THREE.CanvasTexture<RasterCanvas> | null = null;
  ...
  function applyFit() {
    ...
    fitGroup.rotation.set(
      THREE.MathUtils.degToRad(config.rotateX),
      THREE.MathUtils.degToRad(config.rotateY),
      THREE.MathUtils.degToRad(config.rotateZ),
    );
  }
  ...
  const motionQuery =
    typeof ENV.window?.matchMedia === "function"
      ? ENV.window.matchMedia("(prefers-reduced-motion: reduce)")
      : null;
  let reducedMotion = motionQuery
    ? motionQuery.matches
  ...
  const onMotionChange = () => applyReducedMotion(motionQuery!.matches);
  ...
  function resize(next?: CanvasMetrics) {
    if (next) metrics = next;
    const width = Math.max(
      domCanvas ? domCanvas.clientWidth : metrics.width,
      1,
    );
    const height = Math.max(
      domCanvas ? domCanvas.clientHeight : metrics.height,
      1,
    );
    const pr = Math.min(
      domCanvas ? ENV.window?.devicePixelRatio || 1 : metrics.pixelRatio,
      2,
    );
  ...
  const observer = domCanvas ? new ENV.window!.ResizeObserver(() => resize()) : null;
  ...
  const viewObserver =
    domCanvas && typeof ENV.window?.IntersectionObserver === "function"
      ? new ENV.window.IntersectionObserver((entries) => {
      ...
      : null;
  viewObserver?.observe(domCanvas!);
  ...
  return {
    ...
    setTransform(next: AsciiObjectTransform) {
      if (next.scale !== undefined) config.scale = next.scale;
      if (next.xOffset !== undefined) config.xOffset = next.xOffset;
      if (next.yOffset !== undefined) config.yOffset = next.yOffset;
      if (next.rotateX !== undefined) config.rotateX = next.rotateX;
      if (next.rotateY !== undefined) config.rotateY = next.rotateY;
      if (next.rotateZ !== undefined) config.rotateZ = next.rotateZ;
      floatGroup.position.x = config.xOffset;
      floatGroup.position.y = MODEL_LIFT + config.yOffset;
      applyFit();
    },
    ...
    setReducedMotion(value: boolean) {
      // The main thread owns a live media query; letting a host override it
      // there would just fight the listener.
      if (motionQuery || value === reducedMotion) return;
